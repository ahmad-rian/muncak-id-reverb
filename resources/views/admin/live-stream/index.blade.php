                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-3xl font-bold text-error">
                                {{ $streams->where('status', 'live')->count() }}</p>
                        </div>
                        <div class="rounded-full bg-error/10 p-3">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-error" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="card-body">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-3xl font-bold text-gray-400">
                                {{ $streams->where('status', 'offline')->count() }}</p>
                        </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($streams as $stream)
                    <div class="card bg-base-100 shadow-md transition-shadow hover:shadow-lg">
                        <!-- Thumbnail -->
                        <figure class="relative aspect-video bg-gradient-to-br from-gray-800 to-gray-900">
                            @if ($stream->thumbnail_url)
                                <img src="{{ asset('storage/' . $stream->thumbnail_url) }}" alt="{{ $stream->title }}"
                                    class="h-full w-full object-cover">
                            @else
                                <div class="flex h-full w-full items-center justify-center">
                                    <div class="text-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="mx-auto h-16 w-16 text-gray-600" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                                        </svg>
                                        <p class="mt-2 text-sm text-gray-500">No thumbnail</p>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="card-body">
                            ...
                            <div class="card-actions mt-4 justify-end">
                                @if ($stream->user_id === auth()->id())
                                    <a href="{{ route('admin.live-stream.broadcast', $stream->id) }}"
                                    ...
                                    <form action="{{ route('admin.live-stream.destroy', $stream->id) }}" method="POST"
                                ...
                                @else
                                    <a href="{{ route('live-cam.show', $stream->id) }}" class="btn btn-sm btn-outline gap-2"
                                        target="_blank">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
