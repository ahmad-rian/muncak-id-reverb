<x-layout.admin>
  ...
  <div class="flex justify-between gap-4 border-b border-base-300 pb-4">
    <p class="text-2xl font-semibold">{{ $type }} Kecamatan</p>
  </div>
  ...
  <form class="mt-6" action="{{ $route }}" method="post">
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2" x-data="namaSlug">
      ...
      <x-form.input type="text" name="kode" label="Kode Kecamatan" :value="old('kode', $data->kode)" required />
      <x-form.select :route="route('admin.api.kabupaten-kota.select')" name="kode_kabupaten_kota" label="Kabupaten Kota" :value="old('kode_kabupaten_kota', $data->kode_kabupaten_kota)" />
    ...
    <div class="mt-4 flex flex-row-reverse items-center justify-end gap-2">
      ...
      <a class="btn btn-neutral btn-sm" href="{{ route('admin.kecamatan.index') }}">Cancel</a>
    </div>
